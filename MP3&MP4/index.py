...
def descargarAudio(urlU):
    if validaUrl(urlU):
        try:
            ...
            tempArchivo = "archivosTemo"
            out_file = audio.download(output_path=tempArchivo) 
...
def principal():#pagina principal
    ...
    if request.method == 'POST': #Manera de recibir datos con el metodo POST
        ...
        if format == 'Descargar mp3':
            descargarAudio(urlUser)
            ruta_archivo = 'archivosTemp/lejos de ti.mp3'
            # Nombre que se mostrará al usuario al descargar el archivo
            nombre_descarga = 'archivo_descargado.mp3'
